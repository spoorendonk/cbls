...
|----------|---------|-------|
| `elec25`, `elec50` | **bug**, undamped Newton jump | Thomson problem: points on the unit sphere, Coulomb objective `+inf` wherever two coincide. **The objective-encoding defects (#100) are fixed and are no longer the blocker.** What remains: the `.nl` declares no finite variable bounds, so the box is the ±1e9 inf-clamp; random init starts ~1e9 out, and shrinking each variable toward 0 is a huge row improvement — which parks the search exactly on the origin, a stationary point of every row `x²+y²+z²=1`. `float_jump_candidates` offers a single *undamped* Newton step (`x0 - residual/grad`) plus `lb`/`ub`/midpoint; from near the origin that step overshoots to the box bound and is rejected, so the variable freezes. Measured infeasible at violation ≈1 **both with the objective present and with it neutralised**, so it is not objective-related — the earlier "dropping the objective makes elec25 feasible in 20s" claim no longer reproduces. Tightening `inf_clamp` to 1 makes `elec25` feasible at violation 0 post-#100, where pre-#100 it was infeasible at *every* clamp. Fix needs a damped / line-searched Newton candidate. |
...
| `elec25` | infeasible | 243.859 vs BKS 243.813 (0.02%) | Confirms an engine gap, not hardness: a feasible point of near-BKS quality is easy to reach. Originally attributed to #100; that is fixed, and the remaining cause is the undamped Newton jump (see the root-cause table above). |
