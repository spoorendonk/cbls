...
public:
    GLSPInnerSolverHook(const GLSPInstance& inst, const std::vector<int32_t>& seq_handles,
                        const std::vector<std::vector<int32_t>>& lot_handles)
        : inst_(inst) {
        for (auto h : seq_handles) {
            seq_vars_.push_back(handle_to_var_id(h));
        }
        lot_vars_.resize(lot_handles.size());
        for (size_t j = 0; j < lot_handles.size(); ++j) {
            for (auto h : lot_handles[j]) {
                lot_vars_[j].push_back(handle_to_var_id(h));
            }
        }
    }
    ...
    void solve(Model& model, ViolationManager& vm,
        ...
        for (int j = 0; j < J; ++j) {
            for (int t = 0; t < T; ++t) {
                ...
                new_lots[j][t] =
                    (serv_frac > 1e-9) ? inst_.demand[j][t] / serv_frac : inst_.demand[j][t];
            }
        }
        ...
        std::sort(spill_order.begin(), spill_order.end(),
                  [&](int a, int b) { return inst_.holding_cost[a] < inst_.holding_cost[b]; });
        ...
        for (int t = T - 1; t > 0; --t) {
            ...
            for (int ji = 0; ji < J && excess_time > 1e-9; ++ji) {
                ...
                if (new_lots[j][t] < 1e-9) {
                    continue;
                }

                double spill_time = std::min(excess_time, new_lots[j][t] * inst_.process_time[j]);
                ...
                for (int s = t - 1; s >= 0 && spill_units > 1e-9; --s) {
                    ...
                    double spare =
                        std::max(0.0, (inst_.capacity[s] - s_used) / inst_.process_time[j]);
