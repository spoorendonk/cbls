...
inline VerifyResult verify_glsp(const GLSPModel& gm, const GLSPInstance& inst,
                                double tol = 1e-4) {
    ...
    for (int t = 0; t < T; ++t) {
        if ((int)seq_elems[t].size() != J) {
            result.add_error({VerifyError::Kind::Custom,
                "seq[" + std::to_string(t) + "]", (double)J,
                (double)seq_elems[t].size(), "wrong sequence length"});
            continue;
        }
        ...
        for (int k = 0; k < J; ++k) {
            if (sorted[k] != k) {
                result.add_error({VerifyError::Kind::Custom,
                    "seq[" + std::to_string(t) + "]", (double)k,
                    (double)sorted[k], "not a valid permutation of {0..J-1}"});
                break;
            }
        }
    }
        ...
        for (int t = 0; t < T; ++t) {
            ...
            if (lot_val[j][t] < -tol || lot_val[j][t] > ub + tol) {
                result.add_error({VerifyError::Kind::Custom,
                    ...
                    ub, lot_val[j][t], "lot size out of bounds [0," +
                    std::to_string(ub) + "]"});
            }
        }
    }
    ...
    for (int t = 0; t < T; ++t) {
        ...
        if (used > inst.capacity[t] + tol) {
            result.add_error({VerifyError::Kind::ConstraintViolation,
                "capacity[" + std::to_string(t) + "]",
                inst.capacity[t], used,
        ...
        for (int t = 0; t < T; ++t) {
            ...
            if (cum_inv[j][t] < -tol) {
                result.add_error({VerifyError::Kind::ConstraintViolation,
                    ...
                    0.0, cum_inv[j][t],
                    "cumulative demand not satisfied"});
            }
        }
    }
    ...
    for (int j = 0; j < J; ++j) {
        for (int t = 0; t < T; ++t) {
            ...
            double rework_cap = inst.lifetime[j] *
                (inst.capacity[t] / T / M) / inst.rework_time[j];
    ...
    // Print cost breakdown for diagnostics
    fprintf(stderr, "  cost breakdown: changeover=%.1f holding=%.1f rework_hold=%.1f disposal=%.1f total=%.1f\n",
            obj_changeover, obj_holding, obj_rework_hold, obj_disposal, obj);
