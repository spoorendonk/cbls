...
inline Instance make_small() {
    ...
    // Ships
    inst.ships = {
        {"Vessel-A", 11.0, 14.5, 12.0, 14.5, 0.0035, 0.0028, 2500.0, 500.0, 125.0, 25.0, 100.0,
         1800.0, 200.0, 0, 0.0},
        {"Vessel-B", 11.0, 14.5, 12.0, 14.5, 0.0038, 0.0030, 2500.0, 500.0, 125.0, 25.0, 100.0,
         2000.0, 300.0, 2, 2.0},
        {"Vessel-C", 11.0, 14.5, 12.0, 14.5, 0.0033, 0.0026, 2500.0, 500.0, 125.0, 25.0, 100.0,
         1500.0, 150.0, 4, 1.0},
    };
...
inline Instance make_medium() {
    ...
    inst.ships = {
        ...
        {"Vessel-C", 11.0, 14.5, 12.0, 14.5, 0.0033, 0.0026, 2500.0, 500.0, 125.0, 25.0, 100.0,
         1500.0, 150.0, 6, 1.0},
        {"Vessel-D", 11.5, 14.0, 12.5, 14.0, 0.0036, 0.0029, 2200.0, 450.0, 110.0, 22.5, 100.0,
         1600.0, 250.0, 7, 0.0},
        {"Vessel-E", 10.5, 14.5, 11.5, 14.5, 0.0040, 0.0032, 2800.0, 550.0, 140.0, 27.5, 100.0,
         2200.0, 350.0, 4, 3.0},
        {"Vessel-F", 11.0, 14.0, 12.0, 14.0, 0.0034, 0.0027, 2400.0, 480.0, 120.0, 24.0, 100.0,
         1700.0, 180.0, 0, 5.0},
        {"Vessel-G", 11.5, 14.5, 12.5, 14.5, 0.0032, 0.0025, 2600.0, 520.0, 130.0, 26.0, 100.0,
         1900.0, 280.0, 3, 1.0},
    };

    // 30 cargoes: 18 contract + 12 spot
    auto add_cargo = [&](int p, int d, double q, double r, double ps, double pe, double ds,
                         double de, double sl, double sd, bool contract) {
    ...
    for (int r : {0, 2, 3, 4, 5, 6, 7, 8, 10, 11, 13}) {
        for (double d = 0.0; d < inst.planning_horizon_days; d += 10.0) {
            ...
            inst.bunker_options.push_back({r, d, inst.regions[r].hfo_price * price_var,
...
inline Instance make_large() {
    ...
    // Add one more
    base.ships.push_back({"Vessel-H", 11.0, 14.0, 12.0, 14.0, 0.0037, 0.0029, 2300.0, 460.0, 115.0,
                          23.0, 100.0, 1700.0, 220.0, 10, 4.0});
    ...
    for (int r : {0, 3, 6, 7}) {
        for (double d = 60.0; d < 90.0; d += 10.0) {
            double price_var = 1.0 + 0.02 * std::sin(d * 0.3 + r);
            base.bunker_options.push_back({r, d, base.regions[r].hfo_price * price_var,
...
inline Instance load_jsonl(const std::string& path) {
    ...
    for (auto& jr : j["regions"]) {
        inst.regions.push_back({jr["name"], jr["is_eca"], jr["hfo_price"], jr["mgo_price"],
                                jr["port_cost"], jr["bunker_available"]});
    }
    for (auto& jc : j["cargoes"]) {
        inst.cargoes.push_back({jc["pickup_region"], jc["delivery_region"], jc["quantity"],
                                jc["revenue"], jc["pickup_tw_start"], jc["pickup_tw_end"],
                                jc["delivery_tw_start"], jc["delivery_tw_end"],
                                jc["service_time_load"], jc["service_time_discharge"],
                                jc["is_contract"]});
    }
    for (auto& js : j["ships"]) {
        inst.ships.push_back({js["name"], js["v_min_laden"], js["v_max_laden"], js["v_min_ballast"],
                              js["v_max_ballast"], js["fuel_coeff_laden"], js["fuel_coeff_ballast"],
                              js["hfo_tank_max"], js["mgo_tank_max"], js["hfo_safety"],
                              js["mgo_safety"], js["min_bunkering"], js["initial_hfo"],
                              js["initial_mgo"], js["origin_region"], js["available_day"]});
    }
    for (auto& jl : j["legs"]) {
        inst.legs.push_back(
            {jl["from_region"], jl["to_region"], jl["distance"], jl["eca_fraction"]});
    }
    for (auto& jb : j["bunker_options"]) {
        inst.bunker_options.push_back({jb["region"], jb["day"], jb["hfo_price"], jb["mgo_price"]});
    }
