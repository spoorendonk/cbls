...
The C++ suite is **566 ctest tests** over **565 `TEST_CASE`s**: 561 registered
...
in the fast set), so one `TEST_CASE` accounts for two ctest tests. Of the 561,
**6 carry the
...
- `ctest -LE slow` — the other 557 tests, ~12s with `-j`. This is what **pre-commit** runs.
...
5. the `.venv/bin/pytest` line in `README.md` for the Python side (810 tests, 176
...
7. the binding count in **`## Build & Test`** below, in the paragraph explaining
   why the gated build turns `CBLS_BUILD_PYTHON` on ("176 binding tests silently
...
Release (both re-measured 2026-09-26 at 566 tests, `-j12`, on an idle machine).
...
without the flag would leave 176 binding tests silently unrun.
