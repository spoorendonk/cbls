...
The C++ suite is **396 ctest tests** over **395 `TEST_CASE`s**: 391 registered
...
in the fast set), so one `TEST_CASE` accounts for two ctest tests. Of the 391,
**6 carry the
...
- `ctest -LE slow` — the other 387 tests, ~12s with `-j`. This is what **pre-commit** runs.
...
5. the `.venv/bin/pytest` line in `README.md` for the Python side (717 tests, 86
