...
The C++ suite is **388 ctest tests** over **387 `TEST_CASE`s**: 383 registered
...
in the fast set), so one `TEST_CASE` accounts for two ctest tests. Of the 383,
**6 carry the
...
- `ctest -LE slow` — the other 379 tests, ~12s with `-j`. This is what **pre-commit** runs.
...
5. the `.venv/bin/pytest` line in `README.md` for the Python side (695 tests, 84
...
7. the binding count in **`## Build & Test`** below, in the paragraph explaining
   why the gated build turns `CBLS_BUILD_PYTHON` on ("84 binding tests silently
...
without the flag would leave 84 binding tests silently unrun.
