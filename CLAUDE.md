...
The C++ suite is **394 ctest tests** over **393 `TEST_CASE`s**: 389 registered
...
in the fast set), so one `TEST_CASE` accounts for two ctest tests. Of the 389,
**6 carry the
...
- `ctest -LE slow` — the other 385 tests, ~12s with `-j`. This is what **pre-commit** runs.
...
5. the `.venv/bin/pytest` line in `README.md` for the Python side (689 tests, 74
...
7. the binding count in **`## Build & Test`** below, in the paragraph explaining
   why the gated build turns `CBLS_BUILD_PYTHON` on ("74 binding tests silently
...
without the flag would leave 74 binding tests silently unrun.
