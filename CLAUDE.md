...
5. the `.venv/bin/pytest` line in `README.md` for the Python side (684 tests, 69
...
7. the binding count in **`## Build & Test`** below, in the paragraph explaining
   why the gated build turns `CBLS_BUILD_PYTHON` on ("69 binding tests silently
...
without the flag would leave 69 binding tests silently unrun.
