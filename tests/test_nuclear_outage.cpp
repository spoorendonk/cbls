...
TEST_CASE("Incremental dispatch matches full dispatch", "[nuclear]") {
    ...
    for (int trial = 0; trial < 5; ++trial) {
        ...
        if (new_start > inst.outage_latest[o]) {
            new_start = inst.outage_earliest[o];
        }
        starts[o] = new_start;
...
TEST_CASE("Nuclear outage spacing constraints satisfied", "[nuclear]") {
    ...
    for (int u = 0; u < sub.n_units; ++u) {
        ...
        std::sort(outages.begin(), outages.end(), [&](int a, int b) {
            return nm.model.var(handle_to_var_id(nm.s[a])).value <
                   nm.model.var(handle_to_var_id(nm.s[b])).value;
        });
