...
`-j4`, because the box was shared) was **302/302 green in 84.5s** (the fast set was 302 tests at that commit; it is 555 now — this is a record of that run, not a current count), with zero `runtime error` lines, zero AddressSanitizer reports and
...
reports (17 tests carried those tags at that commit; it is 30 now -- this is a
record of that run, not a current count), and the stress harness above — 12 workers, a 12s objective-model arm
