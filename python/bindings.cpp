...
constexpr const char* kParallelSolveDoc =
    ...
    "invoked once per worker, on the same thread and at the same point as\n"
    "model_factory -- so in portfolio mode on each worker's own thread, and in\n"
    "deterministic mode on the calling thread. Both return a shared_ptr, so\n"
